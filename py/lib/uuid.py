...
__date__ = '$Date: 2007/11/24 15:50:13 $'.split()[1].replace('/', '-')
__version__ = '$Revision: 1.2 $'

# Minor modifications by Ed Halley.
#  * memoize getaddr()
#  * add zero-argument UUID() to use uuid4()
#  * add zero-argument uuid() to use uuid4()
...
class UUID (object):
    ...
    def __init__(self, *args):
        """Create a UUID.  If no arguments are given, a randomized UUID is
        returned, like the uuid4() function.  If one argument is given,
        it should be a string representation of the desired UUID value,
        such as an urn: or bracketed hexadecimal string.  Otherwise, provide
        six integers (32-bit time_low, 16-bit time_mid, 16-bit
        time_hi_ver, 8-bit clock_hi_res, 8-bit clock_low, 48-bit node)."""
        if len(args) == 0:
            u = uuid4()
            self.set_bytes(u.get_bytes())
            return
        if len(args) == 1:
...
def getaddr():
    ...
    for dir in ['/sbin', '/usr/sbin', r'c:\windows',
        ...
        if isfile(join(dir, 'ifconfig')):
            __hwaddr = unixgetaddr(join(dir, 'ifconfig'))
        if isfile(join(dir, 'ipconfig.exe')):
            __hwaddr = wingetaddr(join(dir, 'ipconfig.exe'))
        if __hwaddr:
            return __hwaddr
    raise Exception('Could not get a hardware address.')
